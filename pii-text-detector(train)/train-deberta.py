...
# Fix tokenizer parallelism warning and optimize memory
...
class Config:
    ...
    # ========================================
    # DeBERTa Model Configuration - Easy to Change!
    # ========================================
    # Choose one of these DeBERTa variants:
    # MODEL_NAME = "microsoft/deberta-v3-xsmall"  # Fast, good for development
    # MODEL_NAME = "microsoft/deberta-v3-small"   # Balanced speed/performance  
    MODEL_NAME = "microsoft/deberta-v3-base"    # Better performance
    # MODEL_NAME = "microsoft/deberta-v3-large"   # Best performance, slower
    ...
    if "xsmall" in MODEL_NAME:
        ...
        EVAL_BATCH_SIZE = 1  # Reduced from 2 to save memory
        model_size = "large"
    else:
        # Default settings
        EPOCHS = 10
    ...
    OUTPUT_DIR = f"./pii_deberta_{model_size}_v{VER}"
...
def format_input_for_pii_detection(text):
    """
    Create DeBERTa prompt for multilabel PII classification
...
def tokenize_function(examples, tokenizer):
    """Tokenization function for DeBERTa encoder"""
    return tokenizer(
...
def train_model(train_dataset, val_dataset, model, tokenizer, device):
    ...
    # Training arguments optimized for DeBERTa encoder
    training_args = TrainingArguments(
        ...
        warmup_ratio=0.1,  # DeBERTa benefits from warmup
        ...
        max_grad_norm=0.5,  # Reduce gradient norm for stability with small batches
        # Gradient accumulation for larger effective batch size with smaller models
        gradient_accumulation_steps=4 if Config.model_size == "large" else 2,  # Increase for large model to compensate for small batch size
    )
...
def main():
    ...
    print("="*80)
    print("TOKEN-LEVEL PII DETECTION -  DEBERTA TRAINING")
    print("="*80)
    ...
    print(f"\n💡 DeBERTa Info:")
    print(f"   • Microsoft's DeBERTa (Decoding-enhanced BERT with disentangled attention)")
    print(f"   • Enhanced BERT architecture with disentangled attention mechanism")
    print(f"   • Uses relative position encoding for better position understanding")
    print(f"   • Optimized for token classification tasks")
    print(f"   • State-of-the-art performance on NER and token classification")
    print(f"   • Efficient architecture with strong performance on PII detection")
    ...
    try:
        ...
        print(f"   ✅ DeBERTa encoder model loaded successfully for token classification")
    except Exception as e:
    ...
    print(f"   💡 DeBERTa trained for token-level PII detection!")
